...
export function hueFromColor(color: Color | UIColor) {
  ...
  if (nativeColor) {
    ...
    nativeColor.getHueSaturationBrightnessAlpha(
...
export function intoNativeMarkerOptions(options: MarkerOptions) {
  ...
  if (options?.position) {
    opts.position = CLLocationCoordinate2DMake(options.position.lat,
...
export function intoNativeGroundOverlayOptions(options: GroundOverlayOptions) {
  ...
  if (typeof options?.anchorU === 'number' || typeof options?.anchorV === 'number') {
    opts.anchor = CGPointMake(
      options?.anchorU ?? opts.anchor.x,
      options?.anchorV ?? opts.anchor.y,
    );
  }
  ...
  if (options?.position) {
    ...
    opts.position = CLLocationCoordinate2DMake(coords.lat,
      coords.lng);
  }
