...
self.onmessage = (event) => {
    try {
        var Contacts = android.provider.ContactsContract.Contacts, SELECTION = android.provider.ContactsContract.ContactNameColumns.DISPLAY_NAME_PRIMARY;
        var c = helper.android.getContext().getContentResolver().query(Contacts.CONTENT_URI, null, SELECTION + " like ?", ["%" + event.data.searchPredicate + "%"], null);
        if (c.getCount() > 0) {
            var cts = [];
            while (c.moveToNext()) {
