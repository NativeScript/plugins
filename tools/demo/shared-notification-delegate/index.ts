...
export class DemoSharedSharedNotificationDelegate extends DemoSharedBase {
	message: string;
	...
	constructor() {
		...
		if (isIOS) {
			...
			this.notificationCenter.requestAuthorizationWithOptionsCompletionHandler(UNAuthorizationOptions.Alert, (granted, error) => {
				if (!granted) {
